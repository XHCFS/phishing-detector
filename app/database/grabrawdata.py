...
def fetch_phishtank_json() -> list:
    ...
    # If we have an API key, try compressed JSON endpoints first
    if PHISHTANK_API_KEY:
        urls_to_try.extend([
            f"https://data.phishtank.com/data/{PHISHTANK_API_KEY}/online-valid.json.gz",
            f"https://data.phishtank.com/data/{PHISHTANK_API_KEY}/online-valid.json.bz2",
            f"https://data.phishtank.com/data/{PHISHTANK_API_KEY}/online-valid.json",
        ])
    
    # Always include public endpoints as fallback, prioritizing compressed formats
    urls_to_try.extend([
        "https://data.phishtank.com/data/online-valid.json.gz",   # Compressed JSON (preferred)
        "https://data.phishtank.com/data/online-valid.json.bz2", # Alternative compression
        "https://data.phishtank.com/data/online-valid.json",     # Uncompressed JSON
        "https://data.phishtank.com/data/online-valid.csv",      # CSV fallback
    ])
        ...
        try:
            ...
            if r.status_code == 200:
                if url.endswith('.csv'):
                    ...
                    import bz2
                    import json
                    return json.loads(bz2.decompress(r.content).decode('utf-8'))
                else:
                    return r.json()
...
def parse_phishtank_csv(csv_text: str) -> list:
    ...
    for row in reader:
        results.append({
            ...
            'details': []  # CSV format doesn't include details
        })
    return results
...
def map_phishtank_json_obj(o: Dict[str, Any]) -> tuple:
    ...
    # Extract details from the details array (take the first detail if multiple exist)
...
def main():
    ...
    ap.add_argument("--db", default=str(DB_PATH), help="Path to SQLite DB (default: threat_feeds_raw.db)")
